...
void Sha256Class::hashBlock() {
  ...
  for (i = 0; i < 64; i++) {
    ...
    t1 = h;
    t1 += ror32(e, 6) ^ ror32(e, 11) ^ ror32(e, 25); // ÃƒÂ¢Ã‹â€ Ã¢â‚¬Ëœ1(e)
    t1 += g ^ (e & (g ^ f)); // Ch(e,f,g)
    t1 += pgm_read_dword(sha256K + i); // Ki
    t1 += buffer.w[i & 15]; // Wi
    t2 = ror32(a, 2) ^ ror32(a, 13) ^ ror32(a, 22); // ÃƒÂ¢Ã‹â€ Ã¢â‚¬Ëœ0(a)
...
void Sha256Class::pad() {
  // Implement SHA-256 padding (fips180-2 Ãƒâ€šÃ‚Â§5.1.1)
