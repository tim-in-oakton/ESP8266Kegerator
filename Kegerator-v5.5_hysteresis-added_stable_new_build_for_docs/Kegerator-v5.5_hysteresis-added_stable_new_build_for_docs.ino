...
void loop() {
  ...
  // TAP A handling code
  if (TapA_Count == 0) {
    //  do nothing - maybe echo "waiting" - no flow, TapCount==0
  }
  else if (TapA_LastCount > 1 ) {
    // the count just became non-zero, the tap has been pulled
    // call the "tap was pulled routine"
    // note - these sensors have a "jiggle issue" where single ticks are generated by the compressor starting - need to filter out
  ...
  if (TapB_Count == 0) {
  ...
  else if (TapB_LastCount > 1) {
    SetLED(BLUE);  // BLUE is beer dispensing
    SendBeerEvent ( 2, "Pour_started", 0); // the count just became non-zero, the tap has been pulled, tell the cloud
    ...
    if (TapB_EndPourCounter == NumberLoopsToDeclareEOB) {
      if (TapB_Count != 1) {
      //  we've seen no flow for NumberLoopsToDeclareEOB loops, so this meand the tap is off -
      SendBeerEvent ( 2, "Pour_Finished", TapB_Count);   // call the "tap went off routine, send count
      }
      SetLED(GREEN);
      TapB_Count = 0;
      TapB_LastCount = 0;
